...
class SearchType extends AbstractType
{
	...
	public function buildForm(FormBuilderInterface $builder, array $options)
	{
		...
		$builder
			...
			->add('sort', ChoiceType::class, array(
				...
				'multiple' => false,
				'label' => 'Sort',
				'choices' => ['Relevance' => '', 'Year of composition' => 'earliestYear']
			))

			->add('keyword', null, array(
				'required' => false,
				'mapped' => false,
				'attr' => ['placeholder' => 'Search for song titles, poem titles and composers.'],
			))
			...
			->add('poemsTitles', null, array(
				'required' => false,
				'mapped' => false,
				'label' => 'Poem title',
			))

			->add('poems', ChoiceType::class, array(
				'expanded' => false,
				'required' => false,
				'mapped' => false,
				'multiple' => false,
				'label' => 'Poem',
				'choices' => $this->getPoems($em->getRepository('AppBundle:Poem'))
			))
			...
			->add('genres', ChoiceType::class, array(
				...
				'multiple' => false,
				'label' => 'Genre',
				'choices' => $this->getGenres($em->getRepository('AppBundle:Genre'))
			))
			...
			->add('languages', ChoiceType::class, array(
				...
				'multiple' => false,
				'label' => 'Language',
				'choices' => $this->getLanguages($em->getRepository('AppBundle:Lang'))
			))
			
			->add('decades', ChoiceType::class, array(
				'expanded' => false,
				'required' => false,
				'mapped' => false,
				'multiple' => false,
				'label' => 'Decade',
				'choices' => $this->getDecades()
			))

			->add('people', ChoiceType::class, array(
				'expanded' => false,
				'required' => false,
				'mapped' => false,
				'multiple' => false,
				'label' => 'Person',
				'choices' => $this->getPeople($em->getRepository('AppBundle:Person'))
			))
			...
			->add('gender', ChoiceType::class, array(
				...
				'multiple' => false,
				'label' => 'Gender',
				'choices' => $this->getGenders($em->getRepository('AppBundle:Gender'))
			))

			->add('themes', ChoiceType::class, array(
				'expanded' => false,
				'required' => false,
				'mapped' => false,
				'multiple' => false,
				'label' => 'Theme',
				'choices' => $this->getThemes($em->getRepository('AppBundle:Theme'))
			))
			
			->add('earliestYear', null, array(
				'required' => false,
				'mapped' => false,
				'label' => 'Earliest Year',
			))
			
			->add('latestYear', null, array(
				'required' => false,
				'mapped' => false,
				'label' => 'Latest Year',
			))
	...
	public function getPoems($em) {
		//$results = $em->findAll();

		$results = $em->findBy(array(), array('title' => 'ASC'));

		$choices = array();
		foreach($results as $res) {
			$choices[$res->__toString()] = $res->__toString();
		}
		return $choices;
	}
	
	public function getGenres($em) {
		//$results = $em->findAll();

		$results = $em->findBy(array(), array('term' => 'ASC'));
	...
	public function getLanguages($em) {
		//$results = $em->findAll();
	...
	public function getPeople($em) {
		//$results = $em->findAll();

		$results = $em->findBy(array(), array('surname' => 'ASC'));
